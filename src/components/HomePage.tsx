...
const HomePage: React.FC = () => {
    ...
    return (
        <div className="container mx-auto p-6">
                    ...
                    {filteredResults.length === 0 ? (
                        ...
                        filteredResults.map((item, index) => (
                            <div key={index} className="bg-white shadow-md rounded-lg p-4">
                                ...
                                <p className='text-black'>{item.content || item.genre || item.biography}</p>
            ...
            {!searchQuery && (
                <>
                    <h2 className="text-xl text-black font-bold">Latest Work</h2>
                    {works.length > 0 ? (
                        <div className="bg-white shadow-md rounded-lg p-4">
                            <h2 className="text-xl text-black font-bold">{works[0].title}</h2>
                            <p className="text-xl text-black font-bold">Genre: {works[0].genre}</p>
                            <p className="text-xl text-black font-bold">Author: {works[0].authorId}</p>
                            <Link to={`/works`} className="text-blue-500 hover:underline">
                                View Works List
                            </Link>
                        </div>
            ...
            {!searchQuery && (
                <>
                    <h2 className="text-xl text-black font-bold">History</h2>
                    {works.length > 0 ? (
                        <div className="bg-white shadow-md rounded-lg p-4">
                            <h2 className="text-xl text-black font-bold">{historySections[0].title}</h2>
                            <p className="text-xl text-black font-bold">Genre: {historySections[0].description}</p>
                            <p className="text-xl text-black font-bold">Author: {historySections[0].historicalPeriod}</p>
                            <Link to={`/history-sections`} className="text-blue-500 hover:underline">
                                View Works List
                            </Link>
                        </div>
